			...
			<div class="container">
				...
				<div class="box-1">
					...
					<div class="btzy-1 zh-shadow">
						<div class="mpc" v-for="(music, index) in musics" :key="index">
							<img :src="music.cover" class="mpcimg" />
							<div class="mpcxx">
								<p class="mpcxm">歌名：{{music.musictitle}}</p>
								<p class="mpcgs">歌手：{{music.author}}</p>
								<p class="mpchy">播放量：{{music.likes}}</p>
							</div>
							<audio controls class="mp3">
								<source :src="music.addres" type="audio/ogg"></source>
								您的浏览器不支持 audio 元素。
							</audio>
						</div>
					</div>
...
export default {
	...
	created() {
		...
		this.axios.get('http://localhost:8080/music').then(res => {
			...
			this.limit = parseInt(this.musics.length * 0.8);
		});
...
<style scoped>
	...
	.mpc {
		...
		background: rgb(221,221,221); /*--背景色--*/
		/* background-image: url(http://1t.click/b9gW); */
